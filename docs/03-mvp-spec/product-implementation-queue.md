...
| WEB-VERIFY-1 | P0 | Implemented | Separate blocking from advisory owner verification | Blocking questions lead Home as assistant-led resolution cards; advisory questions follow maintenance attention. There is no top-level **Confirm details** destination, and no verification surface appears when there are no questions |
