# VehicleOS Planning Workspace

This workspace holds planning and architecture material outside the code repository.

## Layout

- `docs/01-architecture/`: concrete system architecture and core design decisions
- `docs/02-repo-design/`: proposed code repo structure and the initial design-doc set
- `docs/03-mvp-spec/`: MVP scope, workflows, schemas, and delivery plan
- `docs/05-marketing-site/`: Public landing sitemap and phased page plan
- `docs/deployment/`: Vercel CD setup (preview + production)
- `docs/maintainer/`: Private-repo maintainer notes, CI status, showcase checklist
- `context/source-notes/`: distilled source material that informed the docs
- `context/working-notes/`: assumptions and temporary planning notes

## Deliverables

1. `docs/01-architecture/system-architecture.md`
2. `docs/02-repo-design/repo-structure-and-doc-plan.md`
3. `docs/03-mvp-spec/mvp-technical-spec.md`

The code repository can remain focused on implementation while this workspace keeps longer-form design material nearby but separate.
