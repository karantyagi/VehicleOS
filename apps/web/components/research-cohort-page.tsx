...
export function ResearchRunReview({
  ...
  const sourceGuidanceByRecord = useMemo(() => draft.records.map(researchRecordSourceGuidance), [draft.records]);
  const counts = {
    ...
    <section className="mt-5 rounded-xl border border-border bg-card p-4 shadow-sm">
      ...
      <div className="mt-3 space-y-3">
        {visibleRecords.map(({ record, index }) => {
          const sourceGuidance = sourceGuidanceByRecord[index];
          ...
          return (
            <article key={index} className={`rounded-lg border bg-background ${open ? "border-primary/40 shadow-sm" : "border-border"}`}>
              <button type="button" onClick={() => toggleRecord(index)} className="flex w-full items-start gap-3 p-3 text-left">
                ...
                {complete ? <span className="rounded-full bg-primary/10 px-2 py-1 text-xs font-medium text-primary">Reviewed</span> : sourceGuidance?.code === "work-not-itemized" ? <span className="rounded-full bg-amber-500/10 px-2 py-1 text-xs font-medium text-amber-900 dark:text-amber-200">Source note</span> : sourceGuidance ? <span className="rounded-full bg-amber-500/10 px-2 py-1 text-xs font-medium text-amber-900 dark:text-amber-200">Check with PDF</span> : <span className="rounded-full bg-muted px-2 py-1 text-xs font-medium text-muted-foreground">Not reviewed</span>}
              </button>
              {open ? <div className="border-t border-border px-3 pb-4 pt-3">
                {sourceGuidance ? <div className="flex gap-2 rounded-md border border-amber-500/30 bg-amber-500/5 p-3 text-sm text-amber-950 dark:text-amber-100"><CircleAlert className="mt-0.5 h-4 w-4 shrink-0" aria-hidden /><div><p className="font-medium">{sourceGuidance.title}</p><p className="mt-1 text-xs leading-5">{sourceGuidance.why} {sourceGuidance.nextStep}</p></div></div> : null}
